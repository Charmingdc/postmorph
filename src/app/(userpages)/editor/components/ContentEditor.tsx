...
const ContentEditor = ({ user_id, draft }: PageProps) => {
 ...
 useEffect(() => {
  ...
  if (updateDraftState.type === "error") toast.error(updateDraftState.message);
  ...
  <div className="w-full flex flex-col gap-4 bg-card text-card-foreground p-4 border border-border rounded-2xl shadow-sm">
   ...
   >
    ...
    <input
     ...
     readOnly
    />
    <textarea name="new_content" value={content} className="hidden" readOnly />
   ...
   {loadingProfile ? (
    ...
    profile && <ModifyCountBadge modifyCount={modifyCount} />
   )}
